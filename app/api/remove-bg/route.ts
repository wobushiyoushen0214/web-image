...
const REMOVE_BG_PROMPT =
  "Remove the background completely. Keep only the main subject with clean, sharp edges. Output transparent PNG with alpha channel. Do not change the subject's pose, color, lighting, or details. No background fill, no shadows on background.";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: `下载源图失败：${e instanceof Error ? e.message : String(e)}` },
      { status: 400 },
    );
  }
  ...
  outForm.append("prompt", REMOVE_BG_PROMPT);
  outForm.append("n", "1");
  outForm.append("size", "1024x1024");
  outForm.append("background", "transparent");
  ...
  const upstream = await fetch(`${RELAY_BASE_URL}/images/edits`, {
    ...
    return new Response(
      JSON.stringify({
        error: e.name === "TimeoutError" ? "上游去背景超时（>270s）" : `上游请求失败：${e.message}`,
      }),
